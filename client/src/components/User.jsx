...
const User = ({ handleSetUser, index, friendAndPrifileImage, selected, handleArchiveDeleteCleanChat }) => {
  ...
  const handleOptionClick = async (option) => {
    setClickedBtn(true)
    const data = await handleArchiveDeleteCleanChat(friendAndPrifileImage.friendUserName, option);
    if (data.status) {
      setClickedBtn(false)
  ...
  return (
    <div onClick={handleClick} className={`user h-16 mb-1 w-full flex items-center hover:bg-gray-800 pl-5 border-gray-800 text-white rounded-md cursor-pointer ${selected ? 'bg-gray-800 transition-all' : ''}`}>
      <button  
        className="w-full text-left flex justify-items-start items-center relative"
      >
        <span className="flex items-center justify-center  h-12 w-12 rounded-full border mr-2 bg-[#1c212d] shadow-md">
           <img className='h-12 w-12 rounded-full' src={friendAndPrifileImage.profileImage} alt="" />
        </span>
        {friendAndPrifileImage.friendUserName}
